...
def tridiagonal_split_trial_setup():
...
def tridiagonal_split_trial_setup_cmd(n, seed):
    ...
    rhs, res, sup, dia, sub = tridiagonal_trial_setup({n:d},{seed:d})    
    """.format(n=n,seed=seed))
...
def tridiagonal_trial(n, seed):
    ...
    return timeit.timeit(cmd, setup=tridiagonal_trial_setup_cmd(n, seed), number=int(1e5))

def tridiagonal_split_trial(n, seed):
    """Tridiagonal trial."""
    cmd = "status = tridiagonal_solver(rhs, res, sub, dia, sup)"
    print("Trying n={:d}".format(n))
    return timeit.timeit(cmd, setup=tridiagonal_split_trial_setup_cmd(n, seed), number=int(1e4))
...
if __name__ == '__main__':
    ...
    if os.path.exists(filename):
        ns, times, times_split = np.vsplit(np.load(filename))
    else:
